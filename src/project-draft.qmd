...
Something... see @fig-gender-trends

```{r}
#| label: fig-gender-trends
#| fig-cap: Gender trends ...
#| echo: false
#| message: false
#| warning: false
...
# Gender Trends
 data |>
  ...
  labs(title = "Gender Trends by First Academic Year (Female)",
...
```
Gender ratio has been stable for the last 10 years. The stability of the gender ratio is a positive indication of equal access to ADS services
...
The ratio of ADS serving minority students exceeds that of the overall population within the College of Letters & Science by approximately 10-15%. This suggests that ADS service is valuable and accessible to minority students.
...
Minorities include targeted minorities as well as Other Asians and Native Hawaiians. Thus, it is expected to see the trend in targeted minorities similar to that of minorities.


### Usage Residency Distribution
...
total_contacts <- data %>%
  filter(CONTACT_TYPE_NAME %in% c("Appointment", "Remote Appointment", "E-mail/Mail"), contact_year > 2010 & contact_year < 2024) %>%
  group_by(contact_year, CONTACT_TYPE_NAME) %>%
  summarise(total_contacts = n_distinct(Random_ID))

ggplot(total_contacts, aes(group = CONTACT_TYPE_NAME,x = contact_year, y = total_contacts)) +
  geom_line(aes(color = CONTACT_TYPE_NAME)) +
  geom_point() +
...
ggplot(head(contact_reason_counts_international, 10), aes(x = reorder(CONTACT_REASON1_NAME, n), y = n)) +
  geom_bar(stat = "identity") +
  labs(title = "Contact Reasons of International Students",
       subtitle = "Last 10 Calendar years",
       x = "Contact Reason",
...
ggplot(head(contact_reason_counts_international,10) , aes(x = reorder(CONTACT_REASON1_NAME, n), y = n)) +
  ...
  labs(title = "Contact Reasons of WI resident Students",
       subtitle = "During the last 10 years",
       x = "Contact Reason",
       y = "Number of Occurrences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
        plot.title = element_text(hjust = 0.5)) +
...
unitspec_contact_data <- data_last_10_years |>
  filter(!is.na(UNITSPEC_CONTACT_REASON1_NAME)) 

unitspec_contact_reason_counts <- unitspec_contact_data |>
  group_by(UNITSPEC_CONTACT_REASON1_NAME) |>
  summarize(n = n()) |>
  arrange(desc(n))

top_10_unitspec_reasons <- head(unitspec_contact_reason_counts, 10)

ggplot(top_10_unitspec_reasons, aes(x = reorder(UNITSPEC_CONTACT_REASON1_NAME, n), y = n)) +
  geom_bar(stat = "identity") +
  labs(title = "Top 10 Unit-specific Contact Reasons",
       subtitle = "During the last 10 years",
       x = "UnitSpec Contact Reason",
       y = "Number of Occurrences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
        plot.title = element_text(hjust = 0.5)) +
...
ggplot(top_10_unitspec_reasons, aes(x = reorder(UNITSPEC_CONTACT_REASON1_NAME, n), y = n)) +
  geom_bar(stat = "identity") +
  labs(title = "Top 10 Unit-specific Contact Reasons 1",
       subtitle = "During the last 3 years",
       x = "UnitSpec Contact Reason",
       y = "Number of Occurrences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
        plot.title = element_text(hjust = 0.5)) +
...
# unitspec_contact_data <- data_last_10_years |>
#   filter(!is.na(UNITSPEC_CONTACT_REASON2_NAME)) |>
#   filter(contact_year > 2020)
# 
# unitspec_contact_reason_counts <- unitspec_contact_data |>
#   group_by(UNITSPEC_CONTACT_REASON2_NAME) |>
#   summarize(n = n()) |>
#   arrange(desc(n))
# 
# top_10_unitspec_reasons <- head(unitspec_contact_reason_counts, 10)
# 
# ggplot(top_10_unitspec_reasons, aes(x = reorder(UNITSPEC_CONTACT_REASON2_NAME, n), y = n)) +
#   geom_bar(stat = "identity") +
#   labs(title = "Top 10 Unit-specific Contact Reasons",
#        subtitle = "During the last 3 years",
#        x = "UnitSpec Contact Reason",
...
total_contacts <- data |>
  filter(UNITSPEC_CONTACT_REASON1_NAME %in% c("Credit Load", "Late Drop"), 
         contact_year > 2010 & contact_year < 2024) |>
  group_by(contact_year, UNITSPEC_CONTACT_REASON1_NAME) |>
  summarise(total_contacts = length(Random_ID))

# Plotting
ggplot(total_contacts, aes(group = UNITSPEC_CONTACT_REASON1_NAME, 
                           x = contact_year, y = total_contacts)) +
  geom_line(aes(color = UNITSPEC_CONTACT_REASON1_NAME)) +
  geom_point() +
  labs(title = "Total Number of Contacts each Reason",
       subtitle = "For the last 10 years",
       x = "Year",
       y = "Total Number of Contacts") +
  theme_minimal() +
  scale_x_continuous(breaks = seq(2011, 2023, by = 1)) +
